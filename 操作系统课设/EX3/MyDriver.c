...
static char buffer[1024] = "Here is nhy's device driver!\n"; //数据缓冲区
...
static int __init init_module(void)
{
  ...
  //向系统的字符登记表登记一个字符设备
  result = register_chrdev(DEVICE_NUM, "mydev_nhy", &mydev_fops);
  if (result < 0)
  {
...
{
  unregister_chrdev(device_num, "mydev_nhy"); //字符设备卸载

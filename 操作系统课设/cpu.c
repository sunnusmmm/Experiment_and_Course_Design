...
void CreateCPU(GtkWidget *notebook)
{
    ...
    GtkWidget *CPU_frame2 = gtk_frame_new("CPU利用率");
    GtkWidget *CPU_frame3 = gtk_frame_new("CPU基本信息");

    gtk_table_attach_defaults(GTK_TABLE(table), CPU_frame1, 0, 2, 5, 10);
    gtk_table_attach_defaults(GTK_TABLE(table), CPU_frame2, 0, 10, 0, 5);
    gtk_table_attach_defaults(GTK_TABLE(table), CPU_frame3, 2, 10, 5, 10);
...
void ShowCPUInfo(GtkWidget *vbox)
{
    ...
    sprintf(text, "\t\t缓存\t\t\t：%s", cpu_info.clflush_size);
    ShowLabel(vbox, text);
	
	sprintf(text, "\t\t缓存刷新\t\t：%s", cpu_info.cache_size);
    ShowLabel(vbox, text);
...
void ShowLabel(GtkWidget *vbox, const char *text)
{
    ...
    gtk_misc_set_alignment(GTK_MISC(label), 0, 0.25);

...
void CreateProcess(GtkWidget *notebook)
{
    ...
    char *name[P_COLUMN] = {"PID", "进程名","PPID" ,"状态", "CPU利用率", "占用内存", "优先级"};
    int min_width[P_COLUMN] = {100, 210, 100, 150, 100, 150, 100};
    ...
    for (i = 0; i < P_COLUMN; i++)
    {
        ...
        column = gtk_tree_view_column_new_with_attributes(name[i], renderer, "text", i, NULL); //新建一列
    ...
    g_timeout_add(2000, UpdateProcess, NULL);
}
...
gboolean PopMenu(GtkWidget *widget, GdkEventButton *event, gpointer data)
{
    ...
    if (event->type == GDK_BUTTON_RELEASE && event->button == 3)
    {
        ...
        {
            gtk_tree_model_get(model, &iter, PID_COLUMN, &pid, -1); //在数的相应列中获得该进程的PID
...
gboolean KillProcess(gpointer data)
{
    ...
    if (result == GTK_RESPONSE_YES)
    {
        ...
        {
            // 失败
            ShowInfoDialog("结束进程失败!", 2);
        }
        else
        {
            ShowInfoDialog("结束进程成功!", 1);
        }
    }
...
void ShowInfoDialog(char *info, int k)
{
    GtkWidget *dialog;
	if(k == 1){
		dialog = gtk_message_dialog_new(NULL, GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_INFO, GTK_BUTTONS_OK, "%s", info);
		gtk_window_set_title(GTK_WINDOW(dialog), "信息");
	}else if(k == 2){
		dialog = gtk_message_dialog_new(NULL, GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_ERROR, GTK_BUTTONS_OK, "%s", info);

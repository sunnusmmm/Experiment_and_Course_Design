...
{//对抽象语法树的先根遍历
  ...
  if (T)
	...
	case FOR:
        printf("%*cFor(循环)(%d):\n", indent, ' ',T->pos);
        printf("%*c循环条件:\n", indent + 3, ' ');
        display(T->ptr[0], indent + 3*2); //显示循环条件
        printf("%*c循环体:\n", indent + 3, ' ');
        display(T->ptr[1], indent + 3*2); //显示循环体
        break;
    case FOR_DEC:
		printf("%*c初始化语句:\n", indent, ' ');
        display(T->ptr[0], indent + 3);

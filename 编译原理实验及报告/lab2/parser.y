...
%token DPLUS LP RP LC RC SEMI COMMA LB RB      /*用bison对该文件编译时，带参数-d，生成的.tab.h中给这些单词进行编码，可在lex.l中包含parser.tab.h使用这些单词种类码*/
%token PLUS MINUS STAR DIV MOD ASSIGNOP AND OR NOT IF ELSE WHILE BREAK CONTINUE RETURN SWITCH CASE COLON DEFAULT PLUSASSIGNOP MINUSASSIGNOP STARASSIGNOP DIVASSIGNOP MODASSIGNOP
%token AUTOPLUS AUTOMINUS
...
%token FOR FOR_DEC ARRAY_LIST ARRAY_CALL
...
ForDec: Exp SEMI Exp SEMI Exp {$$=mknode(3, FOR_DEC,yylineno,$1,$3,$5);}
       | Exp SEMI SEMI Exp {$$=mknode(3, FOR_DEC,yylineno,$1,NULL,$4);}
       | Exp SEMI Exp SEMI {$$=mknode(3, FOR_DEC,yylineno,$1,$3,NULL);}
       | Exp SEMI SEMI {$$=mknode(3, FOR_DEC,yylineno,$1,NULL,NULL);}
       | SEMI Exp SEMI Exp {$$=mknode(3, FOR_DEC,yylineno,NULL,$2,$4);}
       | SEMI Exp SEMI  {$$=mknode(3, FOR_DEC,yylineno,NULL,$2,NULL);}
       | SEMI SEMI Exp {$$=mknode(3, FOR_DEC,yylineno,NULL,NULL,$3);}     
